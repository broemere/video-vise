...
class MainWindow(QMainWindow):
    ...
    def update_table(self, folder: str):
        ...
        self.scanner_thread = FileScannerThread(folder, self.lossless_results, self)
        self.scanner_thread.progress.connect(self.update_progress)
        self.scanner_thread.status_text.connect(self.status.setText)
        self.scanner_thread.scan_finished.connect(self.on_scan_finished)
        self.scanner_thread.start()

    @Slot(tuple)
    def on_scan_finished(self, data):
        """Called automatically when the FileScannerThread finishes."""

        # Unpack the data generated by the background thread
        (all_items_to_render, info_map, tail_data, source_map,
         stat_map, sizes, formats, codecs, frames) = data

        # Update our global dictionaries
        self.codecs.update(codecs)
        self.frames.update(frames)
        self.sizes.update(sizes)
        self.formats.update(formats)

        base_folder = Path(self.path_edit.text())

        # --- Populate Table (This is fast and safe on the main thread) ---
        ...
        try:
            ...
            pal = self.table.palette()
            dir_bg_color = QColor("#424242") if pal.color(QPalette.Base).lightness() < 128 else QColor("#eeeeee")
